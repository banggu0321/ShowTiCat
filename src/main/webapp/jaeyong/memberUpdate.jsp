...
<body>
	...
	<div class="contents">
		...
		<form action="memberUpdate.do" method="post">
			<br> <label>아이디 : </label> <input type="text" name="m_id"
				value="${member.m_id}" readonly="readonly"><br>
			<br> <label>비밀번호 : </label> <input type="password" name="m_pw"
				...
				id="pwMsg"></span><br>
			<br> <label>이름 : </label> <input type="text" name="m_name"
				value="${member.m_name}"><br>
			<br> <label>핸드폰 번호 : </label> <input type="text" name="phone"
				pattern="[01]{2}[01679]{1}[0-9]{7,8}" value="${member.phone}">
			<p>-를 빼고 숫자만 입력해주세요</p>

			<label>생일 : </label> <input type="date" name="birth"
				value="${member.birth}"><br>
			<br> <label>성별 : </label> <label> <input type="radio"
				name="gender" value="M" ${member.gender=="M"?"checked":""}>남
			</label> <label> <input type="radio" name="gender" value="W"
				${member.gender=="W"?"checked":""}>여
			</label><br> <label>이메일 : </label> <input type="email" name="email"
				value="${member.email}">
			<p>ex) ShowTiCat@gamil.com</p>

			<label>관심매장 : </label> <select name="items">
				<option value="#"></option>
			...
			<br> <label>포인트 : ${member.point} </label> <input type="hidden"
				name="point" value="${member.point}"> <br> <input
				type="submit" class="btn btn-outline-success" value="수정완료">

			<input type="button" class="btn btn-outline-success"
				m_mm="${member.m_id}" value="회원탈퇴" id="btnDelete">
		</form>
	</div>
	<!-- contents -->
</body>
